...
public class ExpirePointStepConfig {
    ...
    @Bean
    @JobScope
    public Step expirePointStep(StepBuilderFactory stepBuilderFactory,
    ...
    public JpaPagingItemReader<Point> expirePointItemReader(EntityManagerFactory entityManagerFactory,
        ...
        return new JpaPagingItemReaderBuilder<Point>().name("expirePointItemReader")
                                                      .entityManagerFactory(entityManagerFactory)
                                                      .queryString("select p from Point p where p.expireDate < :today and used = 0 and expired = 0")
    ...
    public ItemProcessor<Point, Point> expirePointItemProcessor() {
        return point -> {
            ...
            wallet.minusAmount(point.getAmount());
            return point;
